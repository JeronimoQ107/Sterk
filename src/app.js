...
const state = { view: "home", session: null, exerciseIndex: 0, historySessionId: null, completion: null, updateWaiting: false };
...
function newExerciseDraft(routineId, exercise) {
  ...
  return { sets, status: "pending", entryId: null };
}
...
function renderPrevious(previous) {
  ...
  return `<div class="previous"><span>Última vez</span><strong>${formatDate(previous.recordedAt)}</strong><p>${entrySets(previous).map((set) => `${formatNumber(set.weight)} lb × ${set.reps}`).join(" · ")}</p></div>`;
}
...
function saveExercise() {
  ...
  const entry = { id: current.entryId || uid(), sessionId: state.session.id, recordedAt: now.toISOString(), date: now.toLocaleDateString("es-CO"), time: now.toLocaleTimeString("es-CO", { hour: "2-digit", minute: "2-digit" }), routineId: state.session.routineId, routine: routine().name, exercise: exerciseName(), sets, trackingMode: state.session.trackingMode };
...
function getHistorySessions() {
  ...
  const result = completed.map((session) => ({ ...session, durationSeconds: session.durationSeconds ?? Math.max(0, Number(session.durationMinutes || 0) * 60), entries: entries.filter((entry) => entry.sessionId === session.id), synthetic: false }));
  const orphanGroups = new Map();
  entries.filter((entry) => entry.sessionId !== activeSessionId && (!entry.sessionId || !completedIds.has(entry.sessionId))).forEach((entry) => {
    const key = entry.sessionId || `legacy:${entry.routineId}:${entry.date || entry.recordedAt?.slice(0, 10)}`;
    if (!orphanGroups.has(key)) orphanGroups.set(key, []);
    orphanGroups.get(key).push(entry);
  });
  orphanGroups.forEach((group, id) => {
    const first = group.sort((a, b) => new Date(a.recordedAt) - new Date(b.recordedAt))[0];
    result.push({ id, routineId: first.routineId, routine: first.routine || ROUTINES[first.routineId]?.name || "Entrenamiento", startedAt: first.recordedAt, endedAt: group.at(-1).recordedAt, durationSeconds: 0, exerciseCount: group.length, skippedCount: 0, setCount: group.reduce((sum, entry) => sum + entry.setCount, 0), repCount: group.reduce((sum, entry) => sum + entry.sets.reduce((total, set) => total + set.reps, 0), 0), volume: group.reduce((sum, entry) => sum + entry.volume, 0), entries: group, synthetic: true });
  });
...
function renderHistory() {
  ...
  renderShell(`<section class="screen page-screen">${brand()}<div class="page-title"><p class="eyebrow">TU PROGRESO</p><h2>Historial</h2></div>${sessions.length ? `<div class="history-groups">${sessions.map((session) => `<button class="history-card" data-history-session="${escapeHtml(session.id)}"><span>${formatDate(session.startedAt)} · ${formatTime(session.startedAt)}</span><strong>${escapeHtml(session.routine)}</strong><small>${session.entries.map((entry) => escapeHtml(entry.exercise)).join(" · ")}</small><div class="history-metrics"><b>${session.exerciseCount} ejercicios</b><b>${session.setCount} series</b><b>${formatNumber(session.volume)} lb</b>${session.durationSeconds ? `<b>${formatDuration(session.durationSeconds)}</b>` : ""}</div><i>→</i></button>`).join("")}</div>` : `<div class="empty-state"><strong>Aún no hay historial</strong><p>Completa un entrenamiento y aparecerá aquí.</p></div>`}</section>`, "history");
}
...
app.addEventListener("click", (event) => {
  ...
  if (routineButton) { if (storage.getActiveSession() && !confirm("Esto reemplazará la sesión en curso. ¿Continuar?")) return; const active = storage.getActiveSession(); if (active) storage.deleteSession(active.id); storage.clearActiveSession(); return startRoutine(routineButton.dataset.routine); }
