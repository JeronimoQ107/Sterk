...
function renderSet(set, index, setMode) {
  const controls = (part, side = "") => `<div class="side-row">${side ? `<div class="side-heading"><strong>${sideLabel(side)}</strong>${setMode ? `<button class="set-check" data-toggle-set="${index}" data-side="${side}" aria-label="${part.completed ? "Desmarcar" : "Completar"} ${sideLabel(side)} serie ${index + 1}" aria-pressed="${part.completed}">${part.completed ? "✓" : "○"}</button>` : ""}</div>` : ""}<div class="set-controls"><div><small>PESO · LB</small><button class="weight-value" data-edit-weight="${index}" data-side="${side}" aria-label="Editar peso ${side ? sideLabel(side) : ""} serie ${index + 1}">${formatNumber(part.weight)}<small>lb ↕</small></button></div><div><small>REPETICIONES</small><div class="stepper"><button data-set-reps="${index}" data-side="${side}" data-delta="-1" aria-label="Disminuir repeticiones">−</button><button class="stepper-value" data-edit-reps="${index}" data-side="${side}" aria-label="Editar repeticiones">${part.reps}</button><button data-set-reps="${index}" data-side="${side}" data-delta="1" aria-label="Aumentar repeticiones">+</button></div></div></div>${side ? `<button class="copy-side" data-copy-side="${index}" data-side="${side}">Copiar valores al otro lado</button>` : ""}</div>`;
  return `<article class="set-card ${set.completed ? "done" : ""}"><div class="set-card-heading">${setMode && !set.sides ? `<button class="set-check" data-toggle-set="${index}" aria-label="Completar serie ${index + 1}" aria-pressed="${set.completed}">${set.completed ? "✓" : index + 1}</button>` : `<span class="set-label">SERIE ${index + 1}</span>`}<span>${set.completed ? "COMPLETADA" : ""}</span></div>${set.sides ? SIDES.map((side) => controls(set.sides[side], side)).join("") : controls(set)}</article>`;
}
function sessionControls() {
  return `<div class="session-tools"><button data-action="add-exercises">＋ Añadir ejercicio</button><button data-action="manage-session">Editar entrenamiento</button><button data-action="finish-session">Finalizar entrenamiento</button></div>`;
}
function renderEmptySession() {
  renderShell(`<section class="screen exercise-screen"><header class="detail-header"><button class="icon-button" data-action="leave-session" aria-label="Volver al inicio">←</button>${timer(state.session.startedAt)}</header><h1>Tu sesión sigue abierta</h1><p>Añade el próximo ejercicio cuando estés listo.</p>${sessionControls()}</section>`);
}
...
function renderBuilder() {
  ...
  renderShell(`<section class="screen builder-screen"><header class="detail-header"><button class="icon-button" data-action="cancel-builder">←</button><span>${state.adding ? "AÑADIR EJERCICIOS" : "NUEVO ENTRENAMIENTO"}</span></header><div class="builder-heading"><p class="eyebrow">${selected.length ? categoryLabel(uniqueCategories(selected)).toLocaleUpperCase() : "SELECCIÓN LIBRE"}</p><h1>Elige tus ejercicios</h1></div><label class="search-label">Buscar ejercicio<input id="exercise-search" type="search" placeholder="Nombre en español o inglés" value="${escapeHtml(state.builderQuery)}" autocomplete="off"></label><button class="create-shortcut" data-action="create-custom">＋ Crear ejercicio personalizado</button><div class="muscle-filters">${filters.map((filter) => `<button data-filter="${escapeHtml(filter.id)}" class="${state.builderFilter === filter.id ? "active" : ""}">${escapeHtml(filter.label)}</button>`).join("")}</div><div class="exercise-catalog"><p class="empty-state" id="search-empty" ${visible.some((exercise) => matchesExercise(exercise, state.builderQuery)) ? "hidden" : ""}>No hay resultados. Puedes crear tu ejercicio.</p>${visible.map((exercise) => { const isSelected = state.builderSelection.includes(exercise.id); return `<article data-catalog-id="${escapeHtml(exercise.id)}" ${matchesExercise(exercise, state.builderQuery) ? "" : "hidden"} class="catalog-item ${isSelected ? "selected" : ""}"><button class="catalog-select" data-select-exercise="${escapeHtml(exercise.id)}"><span class="selection-mark">${isSelected ? "✓" : "+"}</span><span><strong>${escapeHtml(exercise.name)}</strong><small>${escapeHtml(MUSCLE_GROUPS[exercise.muscleGroup] || exercise.muscleGroup)} · ${escapeHtml(CATEGORY_LABELS[exercise.category])}</small></span></button>${exercise.custom ? `<button class="catalog-archive" data-archive-exercise="${escapeHtml(exercise.id)}" aria-label="Archivar ${escapeHtml(exercise.name)}">×</button>` : ""}</article>`; }).join("")}</div><details class="custom-exercise"><summary>＋ Crear ejercicio personalizado</summary><form id="custom-exercise-form"><label>Nombre<input name="name" required maxlength="50" autocomplete="off" value="${escapeHtml(state.builderQuery)}"></label><label>Grupo muscular<select name="muscleGroup">${Object.entries(MUSCLE_GROUPS).map(([id, label]) => `<option value="${id}">${label}</option>`).join("")}</select></label><label>Categoría<select name="category">${CATEGORY_ORDER.map((id) => `<option value="${id}">${CATEGORY_LABELS[id]}</option>`).join("")}</select></label><button type="submit">Guardar ejercicio</button></form></details>${selected.length ? `<section class="selected-exercises"><div class="section-heading"><h2>Tu entrenamiento</h2><span>${selected.length} ${selected.length === 1 ? "EJERCICIO" : "EJERCICIOS"}</span></div>${selected.map((exercise, index) => `<div class="selected-row"><span>${index + 1}</span><strong>${escapeHtml(exercise.name)}</strong><button data-move-exercise="${index}" data-direction="-1" ${index === 0 ? "disabled" : ""} aria-label="Subir">↑</button><button data-move-exercise="${index}" data-direction="1" ${index === selected.length - 1 ? "disabled" : ""} aria-label="Bajar">↓</button><button data-remove-exercise="${escapeHtml(exercise.id)}" aria-label="Quitar">×</button></div>`).join("")}</section>` : ""}<footer class="builder-action"><button class="primary-button" data-action="start-session" ${selected.length ? "" : "disabled"}>${state.adding ? "Añadir a la sesión" : "Comenzar entrenamiento"} <span>→</span></button></footer></section>`);
}
